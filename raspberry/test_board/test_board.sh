...
echo "Testing AruiPi Serial communication"
echo "-----------------------------------"
# configure serial port to Arduino compatible mode
stty -F /dev/ttyAMA0 cs8 -cstopb -parenb 57600 ignbrk -brkint -icrnl -imaxbel -opost -onlcr -isig -icanon -iexten -echo -echoe -echok -echoctl -echoke noflsh -ixon -crtscts 
...
# send ping command to arduino, Arduino should respond 0x2a in ping response
#if `i2cget -y 1 0x2a 0xe0 | grep "2a" 1>/dev/null 2>&1`
if `arduipi --spi --getbyte --hex --data 0xe0 | grep "0x2a" 1>/dev/null 2>&1`
then
